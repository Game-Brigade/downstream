...
public class RopeBridge extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		for (int ii = 0; ii < bodies.size-1; ii++) {
			...
			RevoluteJointDef plankJointDef = new RevoluteJointDef();
			plankJointDef.bodyA = bodies.get(ii).getBody();
			plankJointDef.bodyB = bodies.get(ii + 1).getBody();
			plankJointDef.localAnchorA.set(anchor1);
			plankJointDef.localAnchorB.set(anchor2);
			plankJointDef.collideConnected = false;	
			
			Joint plankJoint = world.createJoint(plankJointDef);
			joints.add(plankJoint);
			//#endregion
		}
