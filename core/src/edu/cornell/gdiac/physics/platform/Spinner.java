...
//        barrier.setBodyType(BodyDef.BodyType.StaticBody);
        bodies.add(barrier);
        ...
        WheelObstacle pivot = new WheelObstacle(x,y,SPIN_PIN_RADIUS);
        pivot.setName(SPINNER_NAME);
	...
	protected boolean createJoints(World world) {
		...
		Obstacle pivot = bodies.get(1);
		
		RevoluteJointDef jointDef = new RevoluteJointDef();
		jointDef.bodyA = barrier.getBody();
		jointDef.bodyB = pivot.getBody();
		jointDef.localAnchorA.set(barrier.getCentroid());
		jointDef.localAnchorB.set(pivot.getCentroid());
