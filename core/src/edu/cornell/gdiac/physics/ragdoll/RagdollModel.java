...
public class RagdollModel extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		//save each part
		Body torso = bodies.get(0).getBody();
		Body head = bodies.get(1).getBody();
		Body leftArm = bodies.get(2).getBody();
		Body rightArm = bodies.get(3).getBody();
		Body leftForeArm = bodies.get(4).getBody();
		Body rightForeArm = bodies.get(5).getBody();
		Body leftThigh = bodies.get(6).getBody();
		Body rightThigh = bodies.get(7).getBody();
		Body leftShin = bodies.get(8).getBody();
		Body rightShin = bodies.get(9).getBody();
		...
		jointDef.localAnchorA.set(0f,-TORSO_OFFSET/2);
		jointDef.localAnchorB.set(0f,TORSO_OFFSET/2);
		...
		jointDef.localAnchorA.set(-ARM_XOFFSET/2,ARM_YOFFSET);
		jointDef.localAnchorB.set(ARM_XOFFSET/2,0);
		...
		jointDef.localAnchorA.set(-THIGH_XOFFSET,-THIGH_YOFFSET/2);
		jointDef.localAnchorB.set(0,THIGH_YOFFSET/2);
		joint = world.createJoint(jointDef);
		joints.add(joint);
		
		//left shin to left thigh
		jointDef.bodyA = leftShin;
		jointDef.bodyB = leftThigh;
