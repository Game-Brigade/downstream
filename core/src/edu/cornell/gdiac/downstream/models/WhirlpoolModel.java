...
public class WhirlpoolModel extends WheelObstacle {
	...
	public void draw(GameCanvas canvas){
		...
		canvas.draw(texture, Color.WHITE, texture.getRegionWidth()/2, 
				texture.getRegionHeight()/2, this.getX()*drawScale.x, this.getY()*drawScale.y, (float)(Math.PI*rot*direction),0.4f, 0.4f);
		rot+=0.02;
	...
	public void rotationPass(PlayerModel koi){
		...
		if(fishVec.hasSameDirection(angleVec) && fishSlope < angleSlope + .01 && fishSlope > angleSlope - .01 && numOfRotations >= 3){

			koi.burst();
			koi.setExitingWhirlpool(true);
			numOfRotations = -1;
		}
		else if(fishVec.hasSameDirection(angleVec) && fishSlope < angleSlope + .01 && fishSlope > angleSlope - .01){
			numOfRotations += 1;
		}
		else{}
